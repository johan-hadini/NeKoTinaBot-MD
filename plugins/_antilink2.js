...
export async function before(m, { conn, isAdmin, isBotAdmin }) {
	if (m.isBaileys && m.fromMe)
        ...
        await conn.sendButton(m.chat, `❐══════❮ 𝗔𝗡𝗧𝗜 - 𝗟𝗜𝗡𝗞𝟮 ❯══════❐\n`, `${isBotAdmin ? '' : 'Por suerte no soy admin, asi que no puedo hacer nada unu'}`, NombreDelBot, ['[ 🚮 DESACTIVAR ANTILINK 2 ]', Prefijo+'apagar antilink2'], m)
        if (isBotAdmin && bot.restrict) {
        	await conn.groupParticipantsUpdate(m.chat, [m.sender], 'remove')
        conn.sendMessage(m.chat, { delete: { remoteJid: m.chat, fromMe: false, id: yid, participant: prt }})
        } else if (!bot.restrict) return m.reply('❐═══❮ 𝗠𝗢𝗗𝗢 - 𝗥𝗘𝗦𝗧𝗥𝗜𝗡𝗚𝗜𝗗𝗢 ❯═══❐\n🌟 𝘗𝘢𝘳𝘢 𝘳𝘦𝘢𝘭𝘪𝘻𝘢𝘳 𝘢𝘤𝘤𝘪𝘰𝘯𝘦𝘴 𝘥𝘦 𝘦𝘭𝘪𝘮𝘪𝘯𝘢𝘤𝘪𝘰𝘯, 𝘮𝘪 𝘤𝘳𝘦𝘢𝘥𝘰𝘳𝘢 𝘵𝘪𝘦𝘯𝘦 𝘲𝘶𝘦 𝘦𝘯𝘤𝘦𝘯𝘥𝘦𝘳 𝘦𝘭 𝘮𝘰𝘥𝘰 𝘳𝘦𝘴𝘵𝘳𝘪𝘯𝘨𝘪𝘥𝘰.')
        }
    return !0
}
