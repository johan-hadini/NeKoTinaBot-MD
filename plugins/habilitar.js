  ...
  switch (type) {
    case 'bienvenida': case 'wlc': case 'welcome': {
      if (!m.isGroup) {
        ...
        global.dfail('admin, 'rowner', m, conn)
        throw false
      }
    ...
    case 'noesclabots': {
      ...
      break
    default:
      if (!/[01]/.test(command)) return await conn.sendMessage(m.chat, { text: 'Aqui tienes la lista de opciones 😀', footer: '\n'+NombreDelBot, title: `\n🍎 ${uwu} ${name}`, buttonText: " Seleccióne aqui ✅ ", sections }, { quoted: {key:{fromMe:!1,participant:`0@s.whatsapp.net`,...(m.chat?{remoteJid:"17608914335@s.whatsapp.net"}:{})},message:{"productMessage":{"product":{"productImage":{"mimetype":"image/jpeg","jpegThumbnail":fs.readFileSync('./multimedia/imagenes/mylogo.jpg')},"title":Propietario,"description":"By Yesenia","currencyCode":"PEN","priceAmount1000":"19000","retailerId":"Ghost","productImageCount":1},"businessOwnerJid":`0@s.whatsapp.net`}}} })
      throw false
  }
  await conn.sendMessage(m.chat, { text: `\n@${etiqueta.replace(/@.+/, '')} ${isEnable ? 'activó' : 'desactivó'} *${type}* exitosamente ${isAll ? 'para este bot' : isUser ? '' : 'para este chat'}\n`, mentions: [m.sender] }, {ephemeralExpiration: 24*3600,quoted: {key : {participant : '0@s.whatsapp.net'},message: {documentMessage: {title: `${isEnable ? 'Se activo exitosamente el funcionamiento *${type}* ✅' : 'Se desactivo exitosamente el funcionamiento *${type}* ❎'}`,jpegThumbnail: fs.readFileSync('./multimedia/imagenes/mylogo.jpg') }}}})
